...
This class provides a second order Backwards derivative class
to a derivative superclass.
...
class Diff(object):
    """Same constructor used in every class, including the
...
def compare_Methods():
    """Prints out a table comparing the Backwards1 and Backwards2 classes
    for the negative exponential function at x = 0 for various
    step sizes."""
    print
    print '%20s %20s %20s %20s %20s' % ('Negative Power of 2', 'First', 'Error', 'Second', 'Error')
    print
    for k in range(15):
        first = Backward1(math.exp, -2**(-k))
        second = Backward2(math.exp, -2**(-k))
        print '%20d %20.4f %20.4f %20.4f %20.4f' % (k, first(0), first(0) -1, second(0), second(0) - 1)
